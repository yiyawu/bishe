...
Page({
  ...
  goBack() {
    ...
    if(this.data.role == "presenter"){
      app.func.postJson('weixinCloseRoom', {
        roomId: this.data.roomId
      }, (res) => {
        console.log(res)
      })
    }
    if (pages.length > 1 && (pages[pages.length - 1].__route__ == 'pages/live-room/live-room/live-room')) {
      wx.navigateBack({
        delta: 1
      });
    }
  ...
  onLoad: function (options) {
    ...
    this.data.useCamera = options.useCamera;
